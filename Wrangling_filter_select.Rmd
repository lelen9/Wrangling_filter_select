...
```
After running the data I found out that there is 8 Batman movies in the data.
...
movies %>%
  ...
  drop_na(budget)
```
i believe it is wrong because it is in the wrong order.
...
movies %>%
  arrange(desc(length))
```
After running the codesi found out that the longest movie is the Cure for Insomnia (5220 minutes). While the oldest movie was the Blacksmith scene (1893). 
...
after using the code above i found out that there is 1248 movies made in 1995.
2. Which Comedies with an MPAA rating, over 120 minutes have a rating above 8?

```{r}
movies %>%
  filter(Comedy == 1, mpaa>0, length > 120, rating > 0)
```
After running the codes, I found that there are 4 movies that fall under those categories. 
...
After running the code above, I see there are 3 Harry Potter films such as Harry ptter and the chamber of Secrets, Harry Potter and the Prisoner of Azkaban and Harry potter and the the Sorcerer's Stone. The prisoner of Azkaban has the highest ratings of (7.8), while Sorcerer's Stone had the least out of the three at (7.2) ratings. The chamber of Secrets has a rating of (7.3).But Sorcerer's Stone has the Highest votes (46741). 

4. What is the highest budget for a movie that is an Animation or Comedy with at least 100 votes?

```{r}
movies %>%
  filter(Comedy == 1 | Animation == 1, votes == 100)
movies %>%
  arrange(desc(budget))
```

After running the code above, I found out the movie that has animation, comedy and at least 100 votes is the Wild Wild West with the budget of 170000000.
